...
export class EngineApi {
    ...
    public async startEngine(configuration: ConfigurationMV2): Promise<void> {
        ...
        for (let i = 0; i < filters.length; i += 1) {
            ...
            lists.push({
                id: filterId,
                content: conversionData ? new FilterList(content, conversionData) : content,
        ...
        if (userrules.content.length > 0) {
            lists.push({
                id: USER_FILTER_ID,
                content: userrules.conversionData
                    ? new FilterList(userrules.content, userrules.conversionData)
                    : userrules.content,
                ignoreCosmetic: false,
                ignoreJS: false,
                ignoreUnsafe: false,
        ...
        if (allowlistRulesList) {
            const allowlistContent = new FilterList(
                allowlistRulesList,
                // this filter list is generated by tswebextension, so its guaranteed to be AdGuard compatible
        ...
        if (stealthModeList) {
            const stealthContent = new FilterList(
                stealthModeList,
                // this filter list is generated by tswebextension, so its guaranteed to be AdGuard compatible
                FilterList.createEmptyConversionData(),
            );
